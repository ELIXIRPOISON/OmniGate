...
**Results:** see [`docs/results/anomaly-eval.md`](../results/anomaly-eval.md). Measured against `qwen2.5:7b` served locally by Ollama, 115 calls and no failures. Heuristics only and with the classification stage both give precision 1.000, recall 0.900 at threshold 0.7: the model changes no decision there. What it does change is threshold sensitivity, holding recall 0.342 at a 0.95 threshold where the heuristics collapse to 0.008, which is what matters to an operator who blocks rather than flags. The seven misses it could recover are all id-enumeration; it labels them `suspicious` correctly but anchors its numeric score to within 0.02 of the heuristic score shown in the envelope, and enforcement keys off the number. Measured latency is 1.29 s per call warm and serial, well above the 800 ms sync budget, which is the evidence behind async being the default.
