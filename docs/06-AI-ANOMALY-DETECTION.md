...
**Results:** see [`docs/results/anomaly-eval.md`](../results/anomaly-eval.md). Measured against `qwen2.5:7b` on Ollama, 115 calls, no failures. Combined precision 1.000, recall 0.900 at threshold 0.7, where the heuristics already catch everything the model catches. The model earns its place at block-level thresholds: at 0.8 the combination reaches recall 0.808 against 0.600 for heuristics alone and 0.475 for the model alone. The second stage is escalate-only (`anomaly/combine.ts`) after a measured run in which a model answering "suspicious / medium" to everything took recall from 0.900 to 0.433 by overwriting confident heuristic findings. Latency is 1.29 s per call warm and serial, far above the 800 ms sync budget, which is the evidence behind async-by-default. The dataset is synthetic and the false-positive rate is therefore unmeasured.
