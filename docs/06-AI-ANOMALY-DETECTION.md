...
**Results:** [`anomaly-eval-csic.md`](../results/anomaly-eval-csic.md) is the real measurement: CSIC 2010, 97,065 real requests, recall 0.210 at precision 1.000 with zero false positives in 72,000 benign rows. [`anomaly-eval.md`](../results/anomaly-eval.md) is the synthetic self-test that reported 0.900 and is kept for the history. Five pattern gaps were found by reading the real misses and each was measured on both classes before being added; a sixth was rejected at precision 0.457. The largest remaining miss category is parameter tampering, which needs per-route schema learning rather than patterns. The model contributes four extra detections out of 1,254 on real data, and the gate caps it at 24.6 percent recall regardless, so the next work is at the gate rather than on the model.
